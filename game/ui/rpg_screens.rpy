...
screen rpg_inventory_contents(battle=False):
    ...
    if not battle and has_skill("predator") and item_count("hipokute") > 0:
        textbutton "Use Predator • Refine a Healing Blob":
            action Function(refine_healing_blob)
...
screen relationships_menu():
    ...
    use rpg_panel(
        "Relationships",
        Hide("relationships_menu")
    ):

        $ met = [
            key
            for key in CHARACTER_DATA
            if character_state(key).get("met")
        ]
            ...
            frame:
                ...
                vbox:
                    ...
                    if person.get("romanceable"):
                        ...
                        bar:
                            value StaticValue(
                                relationship_value(key, "romance"),
                                100
                            )
                            xsize 900
                            ysize 18
                    ...
                    text "[relationship_stage(key).title()] connection":
                        size 20
                        color "#A7BDC5"

        text "Visit people at their current locations to talk or give gifts.":
            size 21
...
screen status_menu():
    ...
    use rpg_panel(
        ...
        text "Immunities: [', '.join(player_immunities) or 'None']":
            size 22

        text "Resistances and learned abilities appear in Skills.":
            size 22

        label "Journey"

        for entry in world_state["history"]:
            text "[event_index(entry) + 1]. [entry.replace('_', ' ').title()]":
                size 23

        for entry in world_state["legacy_events"]:
            text "Earlier save • [entry.replace('_', ' ').title()] (order unknown)":
                size 21
        ...
        if eligible_endings():
            ...
            for ending in eligible_endings():
                textbutton ENDINGS[ending]["title"]:
                    action Function(
        ...
        vbox:
            ...
            if CHARACTER_DATA[character_id].get("romanceable"):
                ...
                if relationship_intent.get(character_id) != "romantic":
                    textbutton "Express romantic interest":
                        action Return("romantic")
                        xfill True

                else:
                    textbutton "Keep things friendly":
                        action Return("friendly")
                        xfill True
...
screen gift_selection(character_id):
    ...
    use rpg_panel(
        ...
        $ gifts = [
            key
            for key, data in ITEM_DATA.items()
            if data.get("gift") and item_count(key) > 0
        ]
        ...
        for key in gifts:
            ...
            textbutton "[name]  ×[item_count(key)]":
                action Return(key)
                xfill True

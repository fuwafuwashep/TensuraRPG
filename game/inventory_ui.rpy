# ============================================================
# GRID INVENTORY + CRAFTING
# ============================================================
# 14 x 7 inventory, 100 px cells, 64 max stack.
# The legacy inventory dictionary is kept synchronized so the rest of the
# existing project can continue using item_count/add_item/remove_item.
...
init -4 python:
    ...
    for _item_id, _item_data in ITEM_DATA.items():
        _item_data.setdefault(
            "max_stack",
            64 if _item_data.get("stackable", True) else 1
        )
    ...
    def inventory_item_footprint(item_id):
        return tuple(
            ITEM_DATA.get(item_id, {}).get("footprint", (1, 1))
        )
    ...
    def inventory_occupied_map(ignore_anchor=None):
        ...
        for anchor, entry in enumerate(store.inventory_slots):
            ...
            for cell in inventory_anchor_cells(
                anchor,
                entry["item"]
            ):
    ...
    def inventory_can_place(
        ...
        occupied = inventory_occupied_map(
    ...
    def migrate_inventory_grid():
        ...
        for item_id, quantity in old_inventory.items():
            ...
            if not _add_item_to_grid(item_id, quantity):
                # Do not destroy an older save if everything cannot fit.
                store.inventory_slots = [None] * INVENTORY_SLOT_COUNT
                return False
    ...
    # --------------------------------------------------------
    # COMPATIBILITY API
    # These override the older dictionary-only helpers from core/inventory.rpy.
    # --------------------------------------------------------
    ...
    def set_item_count(item_id, quantity):
        ...
        snapshot = _copy_slots(store.inventory_slots)
    ...
    # --------------------------------------------------------
    # SAFE SCREEN-ACTION WRAPPERS
    # --------------------------------------------------------
    # Ren'Py Function() actions should return None when we want the current
    # interaction to remain active. The gameplay functions above intentionally
    # return True/False for normal code, so screens call these wrappers instead.

    def inventory_open_prepare():
        migrate_inventory_grid()
    ...
    def inventory_use_item_action(item_id):
        use_item(item_id)
        return None


    def inventory_craft_action(quantity):
        craft_output(quantity)
        return None
    ...
    def _put_into_inventory(
        ...
        if (
            ...
            and target_anchor != ignore_anchor
        ):
    ...
    def craft_output(quantity):
        ...
        if not _consume_craft_material(
            input_item,
            amount
        ):
            return False
        ...
        renpy.notify(
            "Crafted %d x %s."
            % (amount, ITEM_DATA[output_item]["name"])
        )
        ...
        fixed:
            ...
            if inventory_ui_tab == "inventory":
                ...
                draggroup:
                    ...
                    # Only anchor cells draw items. Large items span cells.
                        ...
                        if entry is not None:
                            ...
                            drag:
                                ...
                                frame:
                                    ...
                                    ysize item_h
                                    background Frame(
                                        "images/inventory_ui/inventory_item.svg",
                                        16,
                                        16,
                                        16,
                                        16

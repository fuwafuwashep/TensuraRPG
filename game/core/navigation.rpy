init -5 python:
    ...
    def enter_location(location_id):
        ...
        if location_id == "charybdis_border":
            ...
            store.world_state["choices"]["charybdis_return"] = (
                previous
                if previous in ("side1", "side2")
                else store.side_path_3_from
            )

        store.current_location = location_id

        store.current_region = LOCATIONS[location_id].get(
            "region",
            "West_Jura"
        )
    ...
    def location_actions(location_id):
        ...
        for action in view.get("actions", []):
            ...
            if not conditions_met(
                action.get("visible_when", {})
            ):
                continue

            entry = dict(action)

            entry["enabled"] = conditions_met(
                action.get("when", {})
            )
        ...
        for character_id in CHARACTER_DATA:
            ...
            if character_present(
                ...
                actions.append({
                    "id": "npc_" + character_id,
                    "text": (
                        "Talk to "
                        + CHARACTER_DATA[character_id]["name"]
                    ),
                    "kind": "npc",
        ...
        for event_id in available_events(
            ...
            actions.append({
                "id": "event_" + event_id,
                "text": STORY_EVENTS[event_id]["title"],
                "kind": "event",
                "event": event_id,
                "enabled": True,
            })
        ...
        if (
            ...
            actions.append({
                "id": "track_monsters",
                "text": "Track a monster • Keen Smell",
                "kind": "label",
...
label explore_location:
    ...
    $ automatic_events = available_events(
        automatic=True
    )
    ...
    $ nav_action = resolve_location_action(
        _return
    )
    ...
    # Should never normally reach this.
    jump explore_location